...
| UF-12 | `DiagnosticIncidentTests/testMalformedGuidanceFailureProducesOnlyAllowlistedClassification`, `SentryDiagnosticReporterTests/testHandledEventScrubberRemovesSDKAddedIdentityAndContext` | `GoldenGuideUITests/test_GT_UF12_001_realAmbientUIShowsMalformedPlanFailure` | **Partially green:** the post-harness real-app XCUI passed on M3; Issue #9 records allowlist-only HTTP 200 event `139c9b86601e416e9b59db36a6f0e952`; and this M3 authenticated with a Keychain-held `event:read,project:read` token and retrieved `SERPY-CLICKY-MAC-APP-1` plus the exact event without copying raw content. **Red:** installed-artifact observation remains unproven |
