...
final class GoldenPermissionsAndLifecycleUITests: GoldenUITestCase {
    func test_GT_UF01_001_permissionDenialShowsRecoveryAcrossRelaunch() async throws {
        try await launch(flow: "UF-01")
        XCTAssertTrue(
    ...
    func test_GT_UF02_001_realSettingsClosesAndReopensAsOneWindow() async throws {
        try await launch(flow: "UF-02")
        ...
        application.menuBars.menuBarItems["SERPy"].menus.menuItems["Settings…"].click()
        XCTAssertTrue(application.windows["SERPy Settings"].waitForExistence(timeout: 5))
        XCTAssertEqual(application.windows.matching(identifier: "SERPy Settings").count, 1)
        application.terminate()
        XCTAssertTrue(application.wait(for: .notRunning, timeout: 5))
