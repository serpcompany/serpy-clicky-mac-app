#!/usr/bin/env python3
"""Validate tracked focused real-app XCUI proof contracts for Issue 13."""
...
def discover_proofs(tracked_paths: set[str]) -> list[str]:
    """Return every tracked focused real-app XCUI proof rather than an allowlist."""
...
def main() -> int:
    ...
    if not proof_paths:
        print("ERROR: no tracked focused real-app XCUI proof JSON found", file=sys.stderr)
        return 1
    ...
    if failed:
        return 1
    print(
        "Issue 13 focused real-app XCUI evidence honesty lint: PASS "
